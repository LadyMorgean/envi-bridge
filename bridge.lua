...
if not Bridge.Inventory and Bridge.Framework == 'qb' and (GetResourceState('qb-inventory') ~= 'missing' or GetResourceState('lj-inventory') ~= 'missing' or GetResourceState('aj-inventory') ~= 'missing' or GetResourceState('ax-inventory') ~= 'missing' or GetResourceState('ps-inventory') ~= 'missing') then
    if not Bridge.InventoryName then
        ...
        if GetResourceState('qb-inventory') ~= 'missing' then Bridge.InventoryName = 'qs-inventory' end
...
-- ❕ ORIGEN_INVENTORY ❕
if not Bridge.Inventory and GetResourceState('origen_inventory') ~= 'missing' then
...
-- ❕ OVERRIDE DISPATCH ❕
if GetConvar('bridge:dispatch', '') ~= '' then
    Bridge.Dispatch = GetConvar('bridge:dispatch', '')
end

-- ❕ OVERRIDE DISPATCH NAME ❕
if GetConvar('bridge:dispatchname', '') ~= '' then
    Bridge.DispatchName = GetConvar('bridge:dispatchname', '')
end

-- ❕ PS Dispatch ❕
if not Bridge.Zone and GetResourceState('ps-dispatch') ~= 'missing' then
    if not Bridge.DispatchName then Bridge.DispatchName = 'ps-dispatch' end
    Bridge.Dispatch = 'ps-dispatch'
end

-- ❕ Sonoran CAD ❕
if not Bridge.Zone and GetResourceState('sonorancad') ~= 'missing' then
    if not Bridge.DispatchName then Bridge.DispatchName = 'sonorancad' end
    Bridge.Dispatch = 'sonorancad'
end
...
-- ❕ LOAD CONTROLS ❕
if not Bridge.Disabled['controls'] then
    module(('controls/%s'):format(Bridge.Context))
end
...
-- ❕ LOAD DISPATCH ❕
if not Bridge.Disabled['dispatch'] then
    module(('dispatch/%s/%s'):format(Bridge.Dispatch, Bridge.Context))
end
...
-- ❕ LOAD UI ❕
module(('ui/%s'):format(Bridge.Context))
...
if Bridge.Context == 'client' then
    ...
    AddEventHandler('onClientResourceStart', function(resourceName)
        if resourceName ~= Bridge.Name then return end
        Wait(0)
    end)

...
Framework.OpenStash = function(name)
    name = name:gsub("%-", "_")
    -- qs-inventory stash identifiers should include the 'Stash_' prefix
    if Bridge.InventoryName == 'qs-inventory' and name:sub(1,6) ~= 'Stash_' then
        name = 'Stash_' .. name
    end
    if QBInventory then
        TriggerServerEvent(Bridge.Resource .. ':bridge:OpenStash', name)
    else
...
Framework.OpenShop = function(name)
    if QBInventory then
        TriggerServerEvent(Bridge.Resource .. ':bridge:OpenShop', name)
    else
